...
#
#First, create a Hat class in main.py. The class should take a variable number
#of arguments that specify the number of balls of each color that are in the
#hat. For example, a class object could be created in any of these ways:
#
...
#
#For example, if you want to determine the probability of getting at least two
#red balls and one green ball when you draw five balls from a hat containing
#six black, four red, and three green. To do this, you will perform N
#experiments, count how many times M you get at least two red balls and one
#green ball, and estimate the probability as M/N. Each experiment consists of
#starting with a hat containing the specified balls, drawing several balls, and
#checking if you got the balls you were attempting to draw.
#
...
#
#Hint: Consider using the modules that are already imported at the top. Do not
#initialize random seed within the file.
...
class Hat:
    def __init__(self, *args, **kwargs):
        self.contents = [ball for ball, number in kwargs.items() for _ in range(number)]
    ...
    def draw(self, number):
        ...
        for _ in range(number):
            draw_ball = random.randrange(len(self.contents))
            drawn_balls.append(self.contents[draw_ball])
            self.contents.pop(draw_ball)
        return drawn_balls
...
def experiment(hat, expected_balls, num_balls_drawn, num_experiments):
    ...
    for ball, number in expected_balls.items():
            for _ in range(number):
                expected_sample.append(ball)
    for _ in range(num_experiments):
        test = copy.deepcopy(hat)
        sample = test.draw(num_balls_drawn)
        matched = 0
        for i in range(len(expected_sample)):
            for j in range(len(sample)):
                if expected_sample[i] == sample[j]:
                    matched += 1
                    sample.pop(j)
                    break
        if matched == len(expected_sample):
            successful_draw += 1
    
    return(successful_draw / num_experiments)
